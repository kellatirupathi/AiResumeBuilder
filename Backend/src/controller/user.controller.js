...
const registerUser = async (req, res) => {
  ...
  if (!fullName || !email || !password || !niatId) {
    ...
    return res
      ...
      .json(
        new ApiError(
          400,
          "Please provide all required fields: fullName, niatId, email, and password."
        )
      );
  }
  ...
  try {
    ...
    if (existingUserByEmail) {
      console.log("Registration Failed already registered user with this email");
      return res.status(409).json(new ApiError(409, "User with this email already registered."));
    }

    const existingUserByNiatId = await User.findOne({ niatId });
    if (existingUserByNiatId) {
        console.log("Registration Failed already registered user with this NIAT ID");
        return res.status(409).json(new ApiError(409, "User with this NIAT ID already registered."));
    }
