...
//                       <div className="mb-2">
//                         <div className="text-xs text-gray-500 mb-1">NIAT ID</div>
//                         <div className="text-sm font-medium text-gray-800 font-mono">{user.niatId || "Not Available"}</div>
//                       </div>
...
//                       <div>
//                         <div className="font-medium">{user.fullName || "User"}</div>
//                         <div className="text-xs text-gray-500 font-mono">{user.niatId}</div>
//                         <div className="text-xs text-gray-500">{user.email}</div>
//                       </div>
//                     </div>
//                   </div>
...
import { addUserData } from "@/features/user/userFeatures";
import { motion } from "framer-motion";
import { FaUser, FaSignOutAlt, FaTachometerAlt, FaKey } from "react-icons/fa";
...
function Header({ user }) {
  ...
  const [menuOpen, setMenuOpen] = useState(false);
  const [userDropdownOpen, setUserDropdownOpen] = useState(false);
  const [isChangePasswordOpen, setChangePasswordOpen] = useState(false); // State for the new modal
  const userDropdownRef = useRef(null);
  ...
  return (
    <>
      <motion.header
        ...
        transition={{ duration: 0.5 }}
        id="printHeader"
        className={`fixed top-0 left-0 right-0 z-50 px-6 md:px-10 py-4 transition-all duration-300 ${scrolled ? "bg-white/95 backdrop-blur-sm shadow-md" : "bg-white/80 backdrop-blur-sm"}`}
      >
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <Link to="/" className="flex items-center space-x-2 group">
            <span className="text-xl font-bold bg-gradient-to-r from-emerald-500 to-indigo-600 bg-clip-text text-transparent">
          ...
          <div className="hidden md:flex items-center gap-6">
            {user ? (
              <div className="flex items-center gap-3">
                <Button variant="outline" className="border-2 border-indigo-200 text-indigo-600 hover:bg-indigo-50 hover:border-indigo-300 flex items-center gap-2" onClick={() => navigate("/dashboard")}>
                  <FaTachometerAlt className="w-4 h-4" />Dashboard
                </Button>
                <Button className="bg-gradient-to-r from-rose-500 to-pink-600 hover:from-rose-600 hover:to-pink-700 flex items-center gap-2" onClick={handleLogout}>
                ...
                <div ref={userDropdownRef} className="relative">
                  ...
                  {userDropdownOpen && (
                    <div className="absolute right-0 mt-2 w-64 bg-white rounded-lg shadow-xl border border-gray-200 z-50 py-2 text-left">
                      <div className="px-4 py-2 border-b border-gray-100"><div className="text-gray-900 font-medium">User Profile</div></div>
                      <div className="px-4 py-3">
                        <div className="mb-2"><div className="text-xs text-gray-500 mb-1">Full Name</div><div className="text-sm font-medium text-gray-800">{user.fullName || "Not Available"}</div></div>
                        <div className="mb-2"><div className="text-xs text-gray-500 mb-1">NIAT ID</div><div className="text-sm font-medium text-gray-800 font-mono">{user.niatId || "Not Available"}</div></div>
                        <div className="mb-2"><div className="text-xs text-gray-500 mb-1">Email</div><div className="text-sm font-medium text-gray-800 break-all">{user.email || "Not Available"}</div></div>
                        <button onClick={() => setChangePasswordOpen(true)} className="w-full text-left text-sm text-indigo-600 hover:text-indigo-800 flex items-center gap-2 mt-2 pt-2 border-t border-gray-100">
                          <FaKey className="w-3 h-3" /> Change Password
                        </button>
                      </div>
                      <div className="border-t border-gray-100 px-4 py-2">
                        <button onClick={handleLogout} className="w-full text-left text-sm text-red-600 hover:text-red-800 flex items-center gap-2">
        ...
        {menuOpen && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.3 }} className="md:hidden bg-white shadow-lg rounded-b-lg mt-4 overflow-hidden">
            <nav className="flex flex-col py-2"></nav>
            <div className="border-t border-gray-100 px-6 py-4">
              {user ? (
                <div className="flex flex-col gap-3">
                  {user && (
                    <div className="bg-gray-50 p-4 rounded-lg mb-3">
                      <div className="flex items-center gap-3 mb-3">
                       ...
                       <button onClick={() => { setChangePasswordOpen(true); setMenuOpen(false); }} className="w-full text-left text-sm text-indigo-600 hover:text-indigo-800 flex items-center gap-2 mt-2 pt-2 border-t border-gray-200">
                          <FaKey className="w-3 h-3" /> Change Password
                        </button>
                    </div>
                  )}
                  <Button variant="outline" className="w-full justify-center border-2 border-indigo-200 text-indigo-600 hover:bg-indigo-50 flex items-center gap-2" onClick={() => { navigate("/dashboard"); setMenuOpen(false); }}>
                    <FaTachometerAlt className="w-4 h-4" />Dashboard
                  </Button>
                  <Button className="w-full justify-center bg-gradient-to-r from-rose-500 to-pink-600 hover:from-rose-600 hover:to-pink-700 flex items-center gap-2" onClick={() => { handleLogout(); setMenuOpen(false); }}>
                    <FaSignOutAlt className="w-4 h-4" />Logout
                  </Button>
                </div>
              ) : (
