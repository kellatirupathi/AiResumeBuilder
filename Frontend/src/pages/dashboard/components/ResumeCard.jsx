...
function ResumeCard({ resume, refreshData, viewMode = "grid" }) {
  ...
  const handleShare = () => {
    if (resume.googleDriveLink) {
        navigator.clipboard.writeText(resume.googleDriveLink).then(() => {
  ...
  const handleClone = async (newTitle) => {
    ...
    try {
      await cloneResume(resume._id, newTitle);
      toast.success("Resume Cloned", {
        description: `A copy "${newTitle}" has been created.`
      });
      refreshData();
    } catch (error) {
      toast.error("Failed to clone resume", { description: error.message });
    } finally {
    ...
    return (
      <>
          ...
          <div className="flex items-center flex-1">
            ...
            <div className="p-3 pl-4">
              ...
              <div className="flex items-center gap-3 mt-1 text-gray-500 dark:text-gray-400 text-xs">
                <div className="flex items-center"><Calendar className="h-3 w-3 mr-1" /><span>Last updated: {formatDate(resume.updatedAt)}</span></div>
                <div className="flex items-center" title="Total Views"><Eye className="h-3 w-3 mr-1" /><span>{resume.viewCount || 0} views</span></div>
              </div>
            </div>
          </div>
        ...
        <AlertDialog open={openAlert} onOpenChange={setOpenAlert}>
            <AlertDialogContent className="max-w-md"><AlertDialogHeader><AlertDialogTitle className="text-rose-600 dark:text-rose-400">Delete this resume?</AlertDialogTitle><AlertDialogDescription>This action cannot be undone. This will permanently delete your resume and remove all associated data from our servers.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Cancel</AlertDialogCancel><AlertDialogAction onClick={handleDelete} disabled={deleteLoading} className="bg-rose-600 hover:bg-rose-700 text-white">{deleteLoading ? ( <><svg className="animate-spin -ml-1 mr-2 h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg> Deleting...</>) : "Delete Resume"}</AlertDialogAction></AlertDialogFooter></AlertDialogContent>
        </AlertDialog>
        <CloneResumeModal 
            isOpen={isCloneModalOpen}
            onClose={() => setCloneModalOpen(false)}
            onClone={handleClone}
            originalTitle={resume.title}
