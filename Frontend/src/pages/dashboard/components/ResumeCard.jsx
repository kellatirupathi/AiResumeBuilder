...
import React, { useState, useRef } from "react";
import { Eye, Edit, Trash2, MoreVertical, Calendar, ChevronRight, Share2 } from "lucide-react";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
function ResumeCard({ resume, refreshData, viewMode = "grid" }) {
  ...
  const handleShare = () => {
    const publicLink = `${VITE_APP_URL.replace(/\/$/, '')}/api/pdf/public/${resume._id}`;
    navigator.clipboard.writeText(publicLink).then(() => {
      toast.success("PDF link copied to clipboard!", {
        description: "Anyone with this link can view your resume PDF."
      });
    }).catch(err => {
      toast.error("Failed to copy link", {
        description: "Please try again manually."
        ...
        <div className="flex items-center flex-1">
          ...
          <div className="p-4 pl-6">
            <Link to={`/dashboard/edit-resume/${resume._id}`}>
              <h3 className="text-lg font-semibold text-blue-600 dark:text-blue-400 hover:underline transition-colors">
                {resume.title}
              </h3>
            </Link>
            <div className="flex items-center gap-4 mt-1 text-gray-500 dark:text-gray-400 text-xs">
  ...
  return (
    <div className="group relative flex flex-col h-[280px] rounded-xl shadow-md hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1 bg-white dark:bg-gray-800/90 backdrop-blur-sm border border-gray-100 dark:border-gray-800 overflow-hidden">
      ...
      <div className="p-6 flex-grow">
        <Link to={`/dashboard/edit-resume/${resume._id}`}>
          <h3 className="text-xl font-bold text-blue-600 dark:text-blue-400 hover:underline transition-colors">
            {resume.title}
          </h3>
        </Link>
