...
const FloatingResumeScore = ({ resumeInfo }) => {
  ...
  const calculateAndSetPanelPosition = () => {
    ...
    // Panel dimensions are rough estimates
    const panelWidth = Math.min(650, viewportWidth - 32);
    const panelHeightEstimate = Math.min(viewportHeight * 0.75, viewportHeight - 32);
    const margin = 16; 
    ...
    if (buttonRect.top > viewportHeight / 2) {
      ...
      newStyle.bottom = viewportHeight - buttonRect.top + margin;
    } else {
      // Place below
      newStyle.top = buttonRect.bottom + margin;
    }

    // Horizontal positioning
    if (buttonRect.left > viewportWidth / 2) {
       // Place to the left of the button
      newStyle.right = viewportWidth - buttonRect.right;
    } else {
  ...
  const handleTap = () => {
    // Clear any pending single-click action to prevent double-click behavior
    if (clickTimeoutRef.current) {
      clearTimeout(clickTimeoutRef.current);
      clickTimeoutRef.current = null;
      return; // It's a double-click, so do nothing.
    }
    
    // Set a timeout to handle a single click
    clickTimeoutRef.current = setTimeout(() => {
      calculateAndSetPanelPosition();
      setExpanded(true);
      clickTimeoutRef.current = null;
    }, 250); // A standard delay for double-click detection
  };
  ...
  return (
    <>
      ...
      <motion.div
        ...
        onTap={handleTap}
        dragMomentum={false}
        initial={{ bottom: '1.5rem', left: '1rem' }}
        className={`fixed z-50 flex items-center justify-center ${expanded ? 'opacity-0 pointer-events-none' : 'opacity-100'} transition-opacity duration-300 cursor-grab active:cursor-grabbing`}
      >
        <div
