...
function Dashboard() {
  ...
  const [sortOption, setSortOption] = useState("newest");
  const [showATSModal, setShowATSModal] = useState(false);
  const sortDropdownRef = useRef(null); // Ref for the dropdown
  ...
  useEffect(() => {
    ...
    else if (storedDarkMode === null) setDarkMode(window.matchMedia("(prefers-color-scheme: dark)").matches);
  }, [user]);
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          {[...Array(30)].map((_, i) => (
            <motion.div
              ...
              animate={{ y: [0, -30, 0], opacity: [0.1, 0.3, 0.1], scale: [1, Math.random() * 0.5 + 1, 1] }}
              transition={{ repeat: Infinity, duration: 3 + Math.random() * 5, delay: Math.random() * 2 }}
            />
          ))}
        </div>
        <motion.div 
      ...
      <div className="container mx-auto px-4 pt-16 pb-16 relative z-10 max-w-7xl">
        ...
        >
          <div className="flex items-center">
            ...
            <div>
              <h1 className="text-2xl font-bold tracking-tight">Resume Dashboard</h1>
              <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Manage your professional profile documents</p>
            </div>
          </div>
          <button
            onClick={toggleDarkMode}
            className={`p-2.5 rounded-full ${darkMode ? 'bg-gray-800 hover:bg-gray-700' : 'bg-white hover:bg-gray-100'} shadow-md transition-colors`}
            aria-label="Toggle dark mode"
          >
            {darkMode ? <Sun className="h-5 w-5 text-amber-300" /> : <Moon className="h-5 w-5 text-indigo-600" />}
          </button>
          ...
          <div className="p-6 flex flex-col md:flex-row gap-4 items-center justify-between">
            ...
            <div className="flex items-center gap-4 w-full md:w-auto justify-between md:justify-end">
              <div ref={sortDropdownRef} className="relative">
                ...
                {showSortOptions && (
                  <div 
                    className={`absolute z-20 mt-1 w-48 rounded-md shadow-lg ${
                      darkMode ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-100'
                    }`}
                  >
                    <div className="py-1">
                      <button
                        ...
                        className={`w-full text-left px-4 py-2 text-sm ${sortOption === "newest" ? (darkMode ? "bg-emerald-900/30 text-emerald-400" : "bg-emerald-100 text-emerald-600") : (darkMode ? "text-gray-200 hover:bg-gray-700" : "text-gray-700 hover:bg-gray-100")}`}
                      >
                        <div className="flex items-center">{sortOption === "newest" && <svg className="h-4 w-4 mr-2 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>}<span className={sortOption === "newest" ? "" : "ml-6"}>Newest first</span></div>
                      </button>
                      <button
                        onClick={() => { setSortOption("oldest"); setShowSortOptions(false); }}
                        className={`w-full text-left px-4 py-2 text-sm ${sortOption === "oldest" ? (darkMode ? "bg-emerald-900/30 text-emerald-400" : "bg-emerald-100 text-emerald-600") : (darkMode ? "text-gray-200 hover:bg-gray-700" : "text-gray-700 hover:bg-gray-100")}`}
                      >
                        <div className="flex items-center">{sortOption === "oldest" && <svg className="h-4 w-4 mr-2 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>}<span className={sortOption === "oldest" ? "" : "ml-6"}>Oldest first</span></div>
                      </button>
                      <button
                        onClick={() => { setSortOption("alphabetical"); setShowSortOptions(false); }}
                        className={`w-full text-left px-4 py-2 text-sm ${sortOption === "alphabetical" ? (darkMode ? "bg-emerald-900/30 text-emerald-400" : "bg-emerald-100 text-emerald-600") : (darkMode ? "text-gray-200 hover:bg-gray-700" : "text-gray-700 hover:bg-gray-100")}`}
                      >
                        <div className="flex items-center">{sortOption === "alphabetical" && <svg className="h-4 w-4 mr-2 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>}<span className={sortOption === "alphabetical" ? "" : "ml-6"}>A to Z</span></div>
                      </button>
                    </div>
                  </div>
                )}
              ...
              <div className={`${darkMode ? 'bg-gray-700' : 'bg-gray-100'} rounded-lg flex p-1`}>
                <button onClick={() => setViewMode("grid")} className={`p-2 rounded-md transition-colors ${viewMode === "grid" ? `${darkMode ? 'bg-gray-600' : 'bg-white'} text-emerald-500 shadow-sm` : `${darkMode ? 'text-gray-400 hover:bg-gray-600' : 'text-gray-600 hover:bg-gray-200'}`}`} aria-label="Grid View">
                  <Grid className="h-5 w-5" />
                </button>
                <button onClick={() => setViewMode("list")} className={`p-2 rounded-md transition-colors ${viewMode === "list" ? `${darkMode ? 'bg-gray-600' : 'bg-white'} text-emerald-500 shadow-sm` : `${darkMode ? 'text-gray-400 hover:bg-gray-600' : 'text-gray-600 hover:bg-gray-200'}`}`} aria-label="List View">
                  <List className="h-5 w-5" />
                </button>
              </div>
            </div>
          ...
          {resumeList.length > 0 && !isLoading && !searchQuery && (
            <div className={`px-6 py-4 border-t ${darkMode ? 'border-gray-700 bg-gray-800/50' : 'border-gray-100 bg-gray-50/50'}`}>
              <div className="flex flex-wrap gap-6">
                ...
                <div className="flex items-center">
                  <div className={`h-8 w-8 rounded-lg ${darkMode ? 'bg-emerald-900/60' : 'bg-emerald-100'} flex items-center justify-center mr-3`}>
                    <svg className={`h-4 w-4 ${darkMode ? 'text-emerald-400' : 'text-emerald-600'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                  </div>
                  <div>
                    <div className="text-xl font-bold">{resumeList.length > 0 ? new Date(Math.max(...resumeList.map(r => new Date(r.updatedAt)))).toLocaleDateString() : "-"}</div>
                    <div className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Last Updated</div>
                  </div>
                </div>
                <div className="flex items-center">
                  ...
                  <div>
                    <button onClick={handleOpenATSChecker} className={`text-xl font-bold flex items-center ${darkMode ? 'text-purple-400 hover:text-purple-300' : 'text-purple-600 hover:text-purple-700'}`}>
                      Check Score <svg className="h-4 w-4 ml-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                    </button>
                    <div className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>ATS Compatibility</div>
                  </div>
                </div>
        ...
        <div className="relative">
          {isLoading ? (
            <div className="flex flex-col justify-center items-center h-64 space-y-8">
              <div className="relative">
                <div className={`h-28 w-28 rounded-full border-4 ${darkMode ? 'border-gray-700 border-t-emerald-400 border-r-blue-500/70' : 'border-gray-200 border-t-emerald-500 border-r-blue-600/70'} animate-spin`}></div>
                <div className="absolute inset-0 overflow-hidden rounded-full"><motion.div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent" animate={shimmerAnimation} /></div>
              </div>
            ...
            <>
              {searchQuery && filteredList.length === 0 ? (
                ...
                >
                  ...
                  <h3 className={`text-xl font-medium ${darkMode ? 'text-gray-200' : 'text-gray-700'} mb-2`}>No matching resumes found</h3>
                  <p className={`${darkMode ? 'text-gray-400' : 'text-gray-500'} mb-6`}>Try a different search term or create a new resume</p>
                  <Button onClick={() => setSearchQuery("")} className="rounded-xl bg-gradient-to-r from-gray-500 to-gray-600 text-white hover:from-gray-600 hover:to-gray-700">Clear search</Button>
                </motion.div>
              ) : (
                <AnimatePresence mode="wait">
                  <motion.div
                    ...
                    animate="visible"
                    exit="exit"
                    className={viewMode === "grid" ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8" : "flex flex-col space-y-6"}
                  >
                    {!searchQuery && (<motion.div variants={itemVariants} className={viewMode === "list" ? "w-full" : ""}><AddResume viewMode={viewMode} darkMode={darkMode} /></motion.div>)}
                    {filteredList.map((resume) => (<motion.div key={resume._id} variants={itemVariants} className={viewMode === "list" ? "w-full" : ""}><ResumeCard resume={resume} refreshData={fetchAllResumeData} viewMode={viewMode} darkMode={darkMode} /></motion.div>))}
                  </motion.div>
                </AnimatePresence>
              )}
            </>
          ...
          {!isLoading && resumeList.length === 0 && (
            ...
            >
              ...
              <h3 className={`text-3xl font-bold ${darkMode ? 'text-gray-200' : 'text-gray-800'} mb-4`}>Create Your First Resume</h3>
              <p className={`${darkMode ? 'text-gray-400' : 'text-gray-600'} text-lg mb-10 max-w-md mx-auto`}>Build professional resumes with AI assistance to showcase your skills and experience</p>
              <motion.button 
                onClick={() => document.querySelector('.add-resume-trigger')?.click()}
                className="inline-flex items-center justify-center px-8 py-4 rounded-xl bg-gradient-to-r from-emerald-500 to-blue-600 text-white font-medium shadow-lg hover:shadow-xl transition-all hover:from-emerald-600 hover:to-blue-700 transform hover:-translate-y-1"
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.98 }}
              >
                <Plus className="h-5 w-5 mr-2" />Create Your First Resume
              </motion.button>
            </motion.div>
          )}
        </div>
        {!isLoading && resumeList.length > 0 && (
          ...
          >
            ...
            <div className="flex justify-center gap-4">
              <a href="#" className={`${darkMode ? 'text-blue-400 hover:text-blue-300' : 'text-blue-600 hover:text-blue-700'} transition-colors`}>Help Center</a>
              <span>•</span>
              <a href="#" className={`${darkMode ? 'text-blue-400 hover:text-blue-300' : 'text-blue-600 hover:text-blue-700'} transition-colors`}>Privacy Policy</a>
              <span>•</span>
              <a href="#" className={`${darkMode ? 'text-blue-400 hover:text-blue-300' : 'text-blue-600 hover:text-blue-700'} transition-colors`}>Terms of Service</a>
            </div>
          </motion.div>
        )}
      </div>
