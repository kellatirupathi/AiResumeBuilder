      ...
      <header className="sticky top-0 z-10 flex flex-shrink-0 items-center justify-between border-b border-gray-200 bg-white px-6 py-4 shadow-sm">
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            <Input placeholder="Search by title, name, email..." value={searchInput} onChange={handleSearchChange} className="w-72 pl-10 rounded-full border-slate-200 focus:border-slate-900" />
          ...
          {selected.length > 0 ? (
            <>
              ...
              {selected.length === 1 && (
                <Button variant="outline" className="rounded-full border border-slate-200 text-slate-700 hover:border-slate-300 hover:text-slate-900" onClick={() => setResumeDialog({ open: true, record: selectedResumes[0] })}>
                  <Pencil className="mr-2 h-4 w-4" /> Edit
                </Button>
              )}
              <Button className="rounded-full bg-red-600 text-white hover:bg-red-700" onClick={() => setDeleteDialog({ open: true, records: selectedResumes })}>
                ...
                <thead className="sticky top-0 z-10 bg-slate-50/70 text-slate-500">
                  <tr>
                    <th className="w-12 px-3 py-3 text-center">
                      <input type="checkbox" checked={allSelected} onChange={(e) => toggleAll(e.target.checked)} className="h-4 w-4 rounded border-slate-300 accent-slate-900 focus:ring-slate-900/10" />
                    </th>
                    <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500">Resume Title</th>
                    <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500">Full Name</th>
                    <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500">User Student ID</th>
                    <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500">External</th>
                    <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500">User Email</th>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500">Drive</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  {resumes.map((resume, i) => (
                    <tr key={resume._id} className={`transition-colors hover:bg-slate-50/60 ${i % 2 === 0 ? "bg-white" : "bg-gray-50/50"}`}>
                      <td className="w-12 px-3 py-4 text-center">
                        <input type="checkbox" checked={selected.includes(resume._id)} onChange={(e) => toggleSelect(resume._id, e.target.checked)} className="h-4 w-4 rounded border-slate-300 accent-slate-900 focus:ring-slate-900/10" />
                      </td>
                      ...
                      <td className="whitespace-nowrap px-6 py-4">
                        <button
                          ...
                          className="flex items-center gap-2 text-sm font-medium text-slate-900 hover:text-[#FF4800] hover:underline"
                        >
                          <div className="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full bg-slate-100">
                            <span className="text-xs font-medium text-slate-900">{(resume.user?.fullName || "").charAt(0).toUpperCase()}</span>
                          </div>
                          {resume.user?.fullName || "—"}
                        </button>
                      </td>
                      <td className="whitespace-nowrap px-6 py-4">
                        <span className="rounded-md bg-slate-50 px-2 py-1 font-mono text-xs font-medium text-slate-700">{resume.user?.niatId || "—"}</span>
                      </td>
                      <td className="whitespace-nowrap px-6 py-4">
                      ...
                      <td className="whitespace-nowrap px-6 py-4">
                        {resume.driveOutOfSync ? (
                          ...
                          <a href={resume.googleDriveLink} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-xs text-slate-900 hover:text-[#FF4800] hover:underline">
