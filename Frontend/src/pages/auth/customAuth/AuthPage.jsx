...
function AuthPage() {
  ...
  const [password, setPassword] = useState("");
  const [fullName, setFullName] = useState("");
  const [niatId, setNiatId] = useState(""); // State for NIAT ID
  const [signInError, setSignInError] = useState("");
  const [loading, setLoading] = useState(false);
  const navigate = useNavigate();
  ...
  const handleSignUpSubmit = async (event) => {
    ...
    const niatIdPattern = /^N\d{2}[A-Z]\d{2}[A-Z]\d{4}$/i;
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-indigo-900 via-purple-800 to-pink-700 p-4 overflow-hidden">
      ...
      <div className="w-full max-w-4xl flex flex-col md:flex-row shadow-2xl rounded-3xl overflow-hidden relative z-10 h-auto md:h-[650px]">
        {/* Left panel - branding and information */}
        <div className="bg-gradient-to-br from-emerald-500 to-indigo-600 p-8 md:p-12 text-white md:w-2/5 relative flex flex-col justify-between">
          <div className="absolute inset-0 overflow-hidden">
            <svg className="absolute left-0 top-0 h-full w-full" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 1000" preserveAspectRatio="none">
          ...
          <AnimatePresence mode="wait" className="flex-grow">
            {isSignUp ? (
              ...
              >
                ...
                <form onSubmit={handleSignUpSubmit} className="space-y-4 h-full flex flex-col">
                  <div className="space-y-1">
                    <label htmlFor="fullname" className="text-sm font-medium text-gray-700">Full Name</label>
                    <div className="flex items-center border-2 rounded-lg border-gray-300 p-3 focus-within:border-indigo-500 transition-colors">
                      <FaUser className="text-gray-400 mr-3" />
                      <input id="fullname" type="text" placeholder="Enter your full name" required className="outline-none w-full text-gray-800" value={fullName} onChange={(e) => setFullName(e.target.value)} />
                    </div>
                  </div>
                  
                  <div className="space-y-1">
                    <label htmlFor="niatId" className="text-sm font-medium text-gray-700">NIAT ID</label>
                    <div className="flex items-center border-2 rounded-lg border-gray-300 p-3 focus-within:border-indigo-500 transition-colors">
                      <FaIdCard className="text-gray-400 mr-3" />
                      <input id="niatId" type="text" placeholder="e.g., N24H01A0532" required className="outline-none w-full text-gray-800 uppercase" value={niatId} onChange={(e) => setNiatId(e.target.value)} />
                    </div>
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center border-2 rounded-lg border-gray-300 p-3 focus-within:border-indigo-500 transition-colors">
                      <FaEnvelope className="text-gray-400 mr-3" />
                      <input id="email" type="email" placeholder="Enter your email" required className="outline-none w-full text-gray-800" value={email} onChange={(e) => setEmail(e.target.value)} />
                    </div>
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center border-2 rounded-lg border-gray-300 p-3 focus-within:border-indigo-500 transition-colors">
                      <FaLock className="text-gray-400 mr-3" />
                      <input id="password" type={showPassword ? "text" : "password"} placeholder="Create a password" required className="outline-none w-full text-gray-800" value={password} onChange={(e) => setPassword(e.target.value)} />
                  ...
                  <button type="submit" disabled={loading} className={`w-full mt-4 bg-gradient-to-r from-emerald-500 to-indigo-600 text-white py-3 rounded-lg font-medium flex justify-center items-center gap-2 shadow-md hover:shadow-lg transition-all ${loading ? "opacity-80" : "hover:from-emerald-600 hover:to-indigo-700"}`}>
                    {loading ? <Loader2 className="animate-spin h-5 w-5" /> : <>Create Account <FaArrowRight className="text-xs" /></>}
                  </button>
                </form>
              </motion.div>
            ) : (
              <motion.div key="signin" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.3 }} className="w-full h-full">
                <h2 className="text-2xl font-bold mb-6 text-gray-800">Welcome Back</h2>
                <form onSubmit={handleSignInSubmit} className="space-y-5 h-full flex flex-col">
                  <div className="space-y-2">
                    <label htmlFor="signin-email" className="text-sm font-medium text-gray-700">Email Address</label>
                    <div className="flex items-center border-2 rounded-lg border-gray-300 p-3 focus-within:border-indigo-500 transition-colors">
                      <FaEnvelope className="text-gray-400 mr-3" />
                      <input id="signin-email" type="email" placeholder="Enter your email" required className="outline-none w-full text-gray-800" value={email} onChange={(e) => setEmail(e.target.value)} />
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center border-2 rounded-lg border-gray-300 p-3 focus-within:border-indigo-500 transition-colors">
                      <FaLock className="text-gray-400 mr-3" />
                      <input id="signin-password" type={showPassword ? "text" : "password"} placeholder="Enter your password" required className="outline-none w-full text-gray-800" value={password} onChange={(e) => setPassword(e.target.value)} />
                      <button type="button" onClick={() => setShowPassword(!showPassword)} className="text-gray-400 hover:text-gray-600 transition-colors">
                        {showPassword ? <FaEyeSlash /> : <FaEye />}
                      </button>
                    </div>
                  ...
                  <button type="submit" disabled={loading} className={`w-full bg-gradient-to-r from-emerald-500 to-indigo-600 text-white py-3 rounded-lg font-medium flex justify-center items-center gap-2 shadow-md hover:shadow-lg transition-all ${loading ? "opacity-80" : "hover:from-emerald-600 hover:to-indigo-700"}`}>
                    {loading ? <Loader2 className="animate-spin h-5 w-5" /> : <>Sign In <FaArrowRight className="text-xs" /></>}
                  </button>
                </form>
                
                <p className="mt-8 text-center text-gray-600">
                  Don't have an account?{" "}
                  <button onClick={() => setIsSignUp(true)} className="text-indigo-600 font-medium hover:underline focus:outline-none">
                    Sign Up
                  </button>
                </p>
              </motion.div>
